...
st.markdown("업로드한 CSV 데이터를 분석하여 AI가 질문에 답변합니다.")
    ...
    if st.button("파일 적용 및 인메모리 DB 구축"):
        with st.spinner("CSV 파일을 분석하고 메모리에 벡터 데이터베이스를 구축 중입니다..."):
            try:
                ...
                # 데이터 형변환 처리 (안정적인 컬럼별 변환 방식)
                for col in df.columns:
                    # 'ignore' 옵션에 오타가 없는지 확인하세요
                ...
                st.success("✅ 데이터베이스 구축 완료! 아래에서 자유롭게 질문해보세요.")
